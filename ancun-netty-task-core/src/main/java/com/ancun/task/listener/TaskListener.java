...
public class TaskListener {
     ...
     * @param eventBus 事件总线
     */
    public TaskListener(EventBus eventBus, TaskBus taskBus) {
    ...
    public void pushTaskInQuene(InQueneEvent inQueneEvent){
        ...
        logger.info("文件 ：[{}] 任务进队列执行开始：[{}]", TaskUtil.getValue(task.getParamsMap(), SpringContextUtil.getProperty("FILE_KEY")), beginInQueneTime );

        // 配置文件中设置的任务执行间隔时间
        long duration = Integer.valueOf(SpringContextUtil.getProperty(Constant.DURATION));

        // 计算多少时间内重做=已经重做的次数*重做的时间延迟
        duration = task.getRetryCount() * duration;
